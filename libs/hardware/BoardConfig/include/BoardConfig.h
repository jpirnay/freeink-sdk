...
// X3 keeps the 20 MHz it was validated at. DELIBERATE FORK DEVIATION.
//
// Upstream moved every Xteink profile to the shared 10 MHz above (39606d5). For
// the X4 Pro that is the right call and this fork takes it: that board's
// controller varies by batch and its bring-up is still in progress. The X3 is a
// different case — it is shipped, it is device-validated at 20 MHz (the UC8253
// datasheet maximum for serial writes), and halving its clock roughly doubles
// the plane-write time on a 792x528 panel: ~21 ms -> ~42 ms of the refresh, paid
// on every page turn by users who are not the reason this resync happened.
//
// Not a disagreement with upstream, just a refusal to change a validated board
// as a side effect of a merge done for a different one. Revisit deliberately,
// with an X3 in hand, and delete this if 10 MHz measures the same.
constexpr uint32_t XTEINK_X3_DISPLAY_SPI_HZ = 20000000u;
...
constexpr BoardProfile XTEINK_X3 = {
    ...
    528,
    {8, 10, 21, 4, 5, 6, PIN_UNASSIGNED},
    XTEINK_X3_DISPLAY_SPI_HZ,  // displaySpiHz: 20 MHz — see the constant for why X3 keeps it
